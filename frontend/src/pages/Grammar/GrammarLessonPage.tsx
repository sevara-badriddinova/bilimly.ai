...
const LESSONS: Record<string, {
  ...
  'present-simple': {
    ...
    quiz: [
      { question: 'She ___ to work by bus.', options: ['go', 'goes', 'going', 'gone'], correct: 'goes', explanation: 'With he/she/it we add -s to the verb.' },
      { question: 'I ___ coffee every morning.', options: ['drink', 'drinks', 'drank', 'drinking'], correct: 'drink', explanation: 'With I/you/we/they, the verb has no -s.' },
      { question: 'He ___ English very well.', options: ['speak', 'speaks', 'spoke', 'speaking'], correct: 'speaks', explanation: 'He is third person singular, so we add -s.' },
    ],
  },
  'past-simple': {
    ...
    quiz: [
      { question: 'Yesterday I ___ to the market.', options: ['go', 'goes', 'went', 'gone'], correct: 'went', explanation: 'Go is irregular: go → went.' },
      { question: 'She ___ a letter last week.', options: ['write', 'writes', 'written', 'wrote'], correct: 'wrote', explanation: 'Write is irregular: write → wrote.' },
      { question: 'They ___ football this morning.', options: ['play', 'plays', 'played', 'playing'], correct: 'played', explanation: 'Regular verbs add -ed in Past Simple.' },
    ],
  },
  'present-continuous': {
    ...
    quiz: [
      { question: 'She ___ a book right now.', options: ['read', 'reads', 'is reading', 'was reading'], correct: 'is reading', explanation: 'Right now = Present Continuous: is + reading.' },
      { question: 'I ___ to music at the moment.', options: ['listen', 'am listening', 'listened', 'listens'], correct: 'am listening', explanation: 'At the moment = Present Continuous: am + listening.' },
      { question: 'They ___ football now.', options: ['play', 'played', 'are playing', 'plays'], correct: 'are playing', explanation: 'They + are + playing = Present Continuous.' },
    ],
  },
  'future-tenses': {
    ...
    quiz: [
      { question: 'Look at those clouds — it ___ rain.', options: ['will', 'is going to', 'goes to', 'shall'], correct: 'is going to', explanation: 'Evidence (clouds) suggests a plan/prediction: going to.' },
      { question: 'I decided just now — I ___ have the pizza.', options: ["'m going to", 'will', 'am', 'shall'], correct: 'will', explanation: 'Spontaneous decision = will.' },
      { question: 'She ___ visit London next summer.', options: ['will', 'is going to', 'both are correct', 'neither'], correct: 'both are correct', explanation: 'Both will and going to can express future plans.' },
    ],
  },
  'modal-verbs': {
    ...
    quiz: [
      { question: 'You ___ smoke here. It\'s forbidden.', options: ['can', 'must', "can't", 'should'], correct: "can't", explanation: "Can't expresses prohibition." },
      { question: 'She ___ swim very fast — she\'s a champion.', options: ['must', 'can', 'might', 'should'], correct: 'can', explanation: 'Can expresses ability.' },
      { question: 'You ___ study more if you want to pass.', options: ['can', 'might', 'should', 'must not'], correct: 'should', explanation: 'Should gives advice/recommendation.' },
    ],
  },
};
...
export default function GrammarLessonPage() {
  ...
  if (!lesson) return (
    <div className="p-10 text-center">
      ...
      <button onClick={() => navigate('/grammar')} className="text-sm text-blue-500 underline">← Back to Grammar</button>
    </div>
  );
  ...
  return (
    <div className="p-6 lg:p-10 max-w-3xl mx-auto">
      ...
      {/* Explanation */}
      <motion.div className="rounded-2xl p-6 mb-4" style={{ background: 'white', border: '1px solid rgba(0,0,0,0.07)' }}
        ...
        <div className="rounded-xl p-3 text-sm leading-relaxed" style={{ background: `${lesson.color}10`, color: '#4A6280', border: `1px solid ${lesson.color}25` }}>
          🇺🇿 {lesson.explanationUz}
        </div>
      </motion.div>

      {/* Examples */}
      <motion.div className="rounded-2xl p-6 mb-6" style={{ background: 'white', border: '1px solid rgba(0,0,0,0.07)' }}
        ...
        {!done ? (
          <>
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs" style={{ color: 'rgba(255,255,255,0.4)' }}>{quizIndex + 1} / {lesson.quiz.length}</span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 mb-4">
              {q.options.map(opt => {
                ...
                if (showResult && opt === q.correct) { bg = 'rgba(16,185,129,0.2)'; color = '#6EE7B7'; border = 'rgba(16,185,129,0.4)'; }
                else if (showResult && opt === selected && opt !== q.correct) { bg = 'rgba(239,68,68,0.2)'; color = '#FCA5A5'; border = 'rgba(239,68,68,0.4)'; }
                return (
          ...
          <motion.div className="text-center py-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            <div className="text-4xl mb-3">{score === lesson.quiz.length ? '🏆' : score >= lesson.quiz.length / 2 ? '👍' : '📚'}</div>
            ...
            <div className="flex gap-3 justify-center">
              <button onClick={() => { setQuizIndex(0); setSelected(null); setShowResult(false); setScore(0); setDone(false); }}
