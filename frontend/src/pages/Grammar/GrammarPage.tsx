        ...
        <div className="space-y-8">
            ...
            <div className="grid gap-4 sm:grid-cols-3">
                <Card>
                    ...
                    <div className="mt-3"><Progress value={Math.round((completed / grammarLessons.length) * 100)}/>
                    </div>
                </Card>
                <Card>
            ...
            <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
                {filtered.map((l, i) => {
                    ...
                    const inner = (
                        <Card
                            className={`relative h-full ${locked ? "opacity-60" : "hover:-translate-y-1 hover:border-primary"} ${l.status === "current" ? "border-primary shadow-[6px_6px_0_0_oklch(0.30_0.10_280)]" : ""}`}>
                            <div className="flex items-start justify-between">
                                <IconBadge icon={l.icon}
                                           tone={l.status === "current" ? "primary" : l.status === "completed" ? "secondary" : "accent"}
                                           size="md"/>
                                {locked ? (
                                    <Pill tone="muted"><Lock className="h-3 w-3"/></Pill>
                                ) : l.status === "completed" ? (
                                    <Pill tone="secondary"><CheckCircle2
                                        className="h-3 w-3"/> {t("grammar.statusCompleted")}</Pill>
                                ) : l.status === "current" ? (
                            ...
                            <div className="mt-5 flex items-center gap-2 text-xs text-muted-foreground">
                                ...
                                <span
                                    className="ml-auto inline-flex items-center gap-1">{l.minutes} {t("common.minShort")} · <Zap
                                    className="h-3 w-3"/>{l.xp}</span>
                            </div>
                        </Card>
                    );
                    return (
