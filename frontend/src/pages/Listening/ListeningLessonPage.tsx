...
export default function ListeningLessonPage() {
  ...
  if (!lesson) return (
    <div className="p-10 text-center">
      ...
      <button onClick={() => navigate('/listening')} className="text-sm underline" style={{ color: '#F59E0B' }}>← Back</button>
    </div>
  );
    ...
    <div className="p-6 lg:p-10 max-w-2xl mx-auto">
      ...
      {!done ? (
        <AnimatePresence mode="wait">
          <motion.div key={index} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
            ...
            <div className="rounded-2xl p-6 mb-4 text-center"
              ...
              <p className="text-xs mb-4 font-semibold tracking-widest uppercase" style={{ color: 'rgba(255,255,255,0.4)' }}>
                🎧 Listen carefully
              </p>
              ...
              <div className="flex items-center justify-center gap-1 h-10 mb-5">
                {Array.from({ length: 20 }).map((_, i) => (
                  <motion.div key={i}
                    ...
                    transition={{ duration: 0.4 + Math.random() * 0.4, repeat: speaking ? Infinity : 0 }}
                  />
                ))}
              </div>
              ...
              <button onClick={handlePlay} disabled={speaking}
                ...
                style={{ background: speaking ? 'rgba(245,158,11,0.3)' : 'linear-gradient(135deg, #F59E0B, #D97706)', color: speaking ? '#FCD34D' : 'white' }}>
            ...
            <div className="rounded-2xl p-6" style={{ background: 'white', border: '1px solid rgba(0,0,0,0.07)' }}>
              ...
              {ex.questionUz && <p className="text-xs mb-4 italic" style={{ color: '#94A3B8' }}>{ex.questionUz}</p>}
              ...
              <div className="grid grid-cols-2 gap-2 mb-4">
                {ex.options?.map((opt: string) => {
                  ...
                  if (showResult && opt === ex.correctAnswer) { bg = 'rgba(16,185,129,0.1)'; color = '#059669'; border = 'rgba(16,185,129,0.4)'; }
                  else if (showResult && opt === selected && opt !== ex.correctAnswer) { bg = 'rgba(239,68,68,0.08)'; color = '#DC2626'; border = 'rgba(239,68,68,0.3)'; }
                  else if (!showResult && selected === opt) { bg = 'rgba(245,158,11,0.1)'; border = 'rgba(245,158,11,0.4)'; }
                  return (
                    <button key={opt}
                      onClick={() => hasListened && handleSelect(opt)}
                      className="rounded-xl px-4 py-3 text-sm font-medium text-left transition-all"
                      style={{ background: bg, color, border: `1px solid ${border}`, cursor, opacity: !hasListened ? 0.5 : 1 }}>
                      {opt}
                    </button>
                  );
                })}
        ...
        <motion.div className="rounded-2xl p-8 text-center" style={{ background: '#0D1B2A' }}
          ...
          <div className="text-4xl mb-3">{score === lesson.exercises.length ? '🏆' : score >= lesson.exercises.length / 2 ? '👍' : '📚'}</div>
          ...
          <div className="flex gap-3 justify-center">
            <button onClick={() => { setIndex(0); setSelected(null); setShowResult(false); setScore(0); setDone(false); setHasListened(false); setPlayCount(0); }}
