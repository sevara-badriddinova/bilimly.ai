...
const SCENARIOS = [
  { id: "interview", icon: Briefcase, key: "interview", level: "intermediate", color: "text-primary", bg: "bg-primary/10", prompt: "Tell me about a project you built recently." },
  { id: "cafe", icon: Coffee, key: "cafe", level: "beginner", color: "text-secondary", bg: "bg-secondary/10", prompt: "Could I have a coffee and a croissant, please?" },
  { id: "travel", icon: Plane, key: "travel", level: "beginner", color: "text-accent-foreground", bg: "bg-accent/30", prompt: "Where is the gate for my flight to London?" },
  { id: "academic", icon: GraduationCap, key: "academic", level: "advanced", color: "text-primary", bg: "bg-primary/10", prompt: "In my opinion, online learning requires strong discipline." },
] as const;
        ...
        <div className="grid gap-8 md:grid-cols-[1fr_auto]">
          <div>
            ...
            <div className="mt-5 flex flex-wrap items-center gap-3">
              <GhostButton onClick={speakPrompt}><Volume2 className="h-4 w-4" /> {t("speaking.listen")}</GhostButton>
              <GhostButton onClick={resetPractice}><RotateCcw className="h-4 w-4" /> {t("speaking.retry")}</GhostButton>
              {audioUrl && (
            ...
            {score !== null && (
              ...
              >
                ...
                <div className="mt-3 grid grid-cols-3 gap-3 text-sm">
                  <div><span className="text-muted-foreground">{t("speaking.fluency")}</span><div className="font-semibold">{Math.min(100, score + 4)}%</div></div>
                  <div><span className="text-muted-foreground">{t("speaking.accuracy")}</span><div className="font-semibold">{score}%</div></div>
                  <div><span className="text-muted-foreground">{t("speaking.pronunciation")}</span><div className="font-semibold">{Math.max(0, score - 3)}%</div></div>
                </div>
                <p className="mt-3 text-sm text-muted-foreground">
          ...
          <div className="grid place-items-center">
            ...
            {recordedSeconds > 0 && <p className="mt-1 text-xs text-muted-foreground">{recordedSeconds}s recorded</p>}
          </div>
        </div>
      </Card>
          ...
          {SCENARIOS.map((s, i) => {
            ...
            return (
              <motion.div
                ...
                whileHover={{ y: -3 }}
              >
                <Card className={`flex items-start gap-4 hover:border-primary cursor-pointer ${activeScenario.id === s.id ? "border-primary" : ""}`}>
                  ...
                  <div className="flex-1">
                    ...
                    <button onClick={() => { setActiveScenario(s); resetPractice(); }} className="mt-3 inline-flex items-center gap-1 text-sm font-semibold text-primary hover:underline">
