...
export default function SpeakingLessonPage() {
  ...
  if (!lesson) return (
    <div className="p-10 text-center">
      ...
      <button onClick={() => navigate('/speaking')} className="text-sm underline" style={{ color: '#8B5CF6' }}>← Back</button>
    </div>
  );
      ...
      {!done ? (
        <AnimatePresence mode="wait">
          <motion.div key={index} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
            ...
            {/* Phrase card */}
            <div className="rounded-2xl p-6 mb-4" style={{ background: 'white', border: '1px solid rgba(139,92,246,0.15)' }}>
              {ex.phoneticSpelling && (
                <p className="text-xs font-mono mb-2" style={{ color: '#A78BFA' }}>{ex.phoneticSpelling}</p>
              )}
              ...
              {ex.questionUz && <p className="text-xs mt-0.5" style={{ color: '#94A3B8' }}>{ex.questionUz}</p>}
              {ex.explanation && (
                <p className="text-xs mt-3 leading-relaxed p-3 rounded-xl" style={{ background: 'rgba(139,92,246,0.06)', color: '#64748B' }}>
                  💡 {ex.explanation}
                </p>
              )}
            </div>
            ...
            <div className="rounded-2xl p-6" style={{ background: '#0D1B2A' }}>
              ...
              <div className="flex items-center gap-3 mb-5">
                {(['listen', 'record', 'result'] as Step[]).map((s, i) => (
                  <div key={s} className="flex items-center gap-2">
                    ...
                    <span className="text-xs capitalize" style={{ color: step === s ? 'white' : 'rgba(255,255,255,0.4)' }}>{s}</span>
              ...
              {step === 'listen' && (
                <div className="text-center">
                  ...
                  <button onClick={handleListen} disabled={speaking}
                    ...
                    style={{ background: speaking ? 'rgba(139,92,246,0.3)' : 'linear-gradient(135deg, #8B5CF6, #7C3AED)', color: 'white' }}>
              ...
              {step === 'record' && (
                <div className="text-center">
                  ...
                  {micError && (
                    <p className="text-xs mb-3 p-2 rounded-lg" style={{ background: 'rgba(239,68,68,0.15)', color: '#FCA5A5' }}>
                      {micError}
                    </p>
                  )}
                ...
                <div>
                  ...
                  {srSupported && score !== null ? (
                    <div className="mb-4 text-center">
                      <div className="text-4xl font-extrabold mb-1" style={{ color: score >= 70 ? '#6EE7B7' : score >= 40 ? '#FCD34D' : '#FCA5A5' }}>
                        {score}%
                      </div>
                    ...
                    <div className="mb-4 p-3 rounded-xl text-center"
                      ...
                      <p className="text-xs" style={{ color: 'rgba(255,255,255,0.5)' }}>
                        Listen to your recording above. Speech recognition is only available in Chrome.
                      </p>
                    </div>
                  ) : null}

                  {/* Compare */}
                  <div className="mb-4 p-3 rounded-xl" style={{ background: 'rgba(139,92,246,0.1)', border: '1px solid rgba(139,92,246,0.2)' }}>
                    <p className="text-xs mb-1" style={{ color: 'rgba(255,255,255,0.4)' }}>Target phrase:</p>
        ...
        <motion.div className="rounded-2xl p-8 text-center" style={{ background: '#0D1B2A' }}
          ...
          <p className="text-sm mb-6" style={{ color: 'rgba(255,255,255,0.5)' }}>
            You practiced {lesson.exercises.length} phrases.{srSupported ? ` Average score: ${avgScore}%.` : ' Keep it up!'}
          </p>
          <div className="flex gap-3 justify-center">
            <button onClick={() => { setIndex(0); setStep('listen'); setScore(null); setTotalScore(0); setDone(false); }}
